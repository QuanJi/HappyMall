    ...
    <div class="container">
        ...
        <div class="checkout-right">
            <table class="timetable_sub">
                ...
                <c:forEach items="${checkout}" var="goods" varStatus="index">
                <tr class="${goods.id}">
                    <td class="invert">${goods.id}</td>
                    <td class="invert-image"><a href="#"><img style="height: 170px;width: 200px" src="${goods.goodsImage}" alt=" " class="img-responsive" /></a></td>
                    <td class="invert">
                        <div class="quantity">
                            <div class="quantity-select">
                                <div class="entry value-minus" onclick="subNum(${goods.id},${goods.goodsPrice})">&nbsp;</div>
                                <div class="entry value" id="1${goods.id}" ><span>1</span></div>
                                <div class="entry value-plus active" onclick="addNum(${goods.id},${goods.goodsPrice},${index.index})" >&nbsp;</div>
                            </div>
                        </div>
                    </td>
                    <td class="invert">${goods.goodsName}</td>

                    <td class="GoodsPrice" id="${goods.id}">${goods.goodsPrice}</td>
                    <td class="invert">
                        <div class="rem">
                            <div class="close1" onclick="deleteCheckoutID(${goods.id})" > </div>
                        </div>
                ...
                <!--quantity-->
                <script>
                    function addNum(id,price,element){
                      /*  var divUpd = element.parent().find('.value'), newVal = parseInt(divUpd.text(), 10)+1;
                        divUpd.text(newVal);*/
                        $("#1"+id).text(parseInt($("#1"+id).text())+1);
                       $("#"+id).text(parseFloat($("#"+id).text())+price);
                        var total=0;
                        $(".GoodsPrice").text(function (index,content) {
            ...
            <h3 >总计：<span id="total">5888.0</span></h3><br>
            <script>
                var total=0;
                $(".GoodsPrice").text(function (index,content) {
                    total+=parseFloat(content);
...
<script>
   function deleteCheckoutID(id){
       var cookieList=document.cookie.split(";");
       for(var i=0;i<cookieList.length;i++){
           var element=cookieList[i].split("=");
           if(element[0].indexOf("idList")!=-1){
               if(element[1].indexOf(id)!=-1){
                   var allID=element[1].split("#");
                   console.log(allID)
                   var newIDList="";
                   for(var j=0;j<allID.length-1;j++){
                       if(allID[j]!=id){
                           newIDList+=allID[j]+"#"
                       }
                   }
                   ...
                   date.setTime(date.getTime()+10*60*60*24*1000);
                    document.cookie="idList="+newIDList+";expires="+date.toUTCString()+";path=/";
...
<script type="text/javascript">
    ...
    $(".button").click(function () {
        ...
        var info="";
        <%List<Goods> list=(List<Goods>)request.getAttribute("checkout");
        for(int i=0;i<list.size();i++){

        %>
        info+=<%=list.get(i).getId()%>+","+$("#1"+<%=list.get(i).getId()%>).text()+";";
        <%
        }%>
        $.ajax({
